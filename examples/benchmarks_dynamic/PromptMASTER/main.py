...
from qlib.contrib.meta.incremental.model import MetaModelRolling, MASTERManager
...
class Incremental:
    # lqa: Indeed, `Incremental` serves as a benchmark for the incremental learning task.
    """
    Example:
    ...
    def __init__(
            ...
            d_feat = 158, d_model = 256, t_nhead = 4, s_nhead = 2, T_dropout_rate=0.5, S_dropout_rate=0.5,
            beta=10, gate_input_end_index=221, gate_input_start_index=158,
            n_epochs=40, lr = 8e-6, GPU = 3, seed = 0, train_stop_loss_thred = 0.95,
            m_prompts = 10, n_prompts = 5, len_prompts = 5, lamb = 0.5,
    ):
        """
        Args:
            data_dir (str):
        ...
        with open("./workflow_config_master_Alpha158.yaml", "r") as f:
            self.basic_task = yaml.safe_load(f)
        self.reload_tag = reload_tag # if None, train from scratch; otherwise, reload checkpoints from the previous experiments
        self.save = save # whether to save the checkpoints
        self.data_dir = data_dir # data directory
        self.market = self.basic_task['market'] # market name
        self.benchmark = self.basic_task['benchmark']
        # benchmark index
        ...
        self.step = step # rolling step
        self.horizon = horizon # decide the label: 
        self.forecast_model = forecast_model  # downstream forecasting models' type
        self.alpha = alpha
        self.tag = tag # to distinguish experiment id
        # self.rank_label = rank_label # expect_label_processor = "CSRankNorm" if self.rank_label else "CSZScoreNorm" in Benchmark
        # self.lr = lr
        # self.lr_da = lr_da
        ...
        # self.num_head = num_head # number of transformation heads
        # self.temperature = tau # softmax temperature
        # self.first_order = first_order # whether use first-order approximation version of MAML
        # self.naive = naive # if True, degrade to naive incremental baseline; if False, use DoubleAdapt
        # self.adapt_x = adapt_x # whether adapt features
        # self.adapt_y = adapt_y # whether adapt labels
        # self.reg = reg # regularization strength for DoubleAdapter
        self.weight_decay = weight_decay # L2 regularization of the (Adam) optimizer
        self.not_sequence = self.forecast_model in ["MLP", 'Linear'] and self.alpha == 158 # 
        # self.h_path = h_path # prefetched handler file path to load
        # use Benchmark.basic_task() to get the basic task configuration
        ...
        #     test_end=test_end,
        # ).basic_task()
        self.begin_valid_epoch = begin_valid_epoch # accelerate offline training by leaving out some valid epochs
        self.preprocess_tensor = preprocess_tensor # if False, temporally transform each batch from `numpy.ndarray` to `torch.Tensor` (slow, not recommended)
        self.use_extra = use_extra # if True, use extra segments for upper-level optimization (not recommended when step is large enough)
        ...
        # self.meta_exp_name = f"{self.market}_MASTER_alpha158_horizon{self.horizon}_step{self.basic_task['dataset']['kwargs']['step_len']}_mprompts{self.m_prompts}_nprompts{self.n_prompts}_lenprompts{self.len_prompts}"
        self.infer_exp_name = self.meta_exp_name + "_backtest"
        R.set_uri((DIRNAME / 'mlruns').as_uri()) # set the uri of the experiment

    @property
    def meta_exp_name(self):
        return f"{self.market}_MASTER_alpha158_horizon{self.horizon}_step{self.basic_task['dataset']['kwargs']['step_len']}_mprompts{self.m_prompts}_nprompts{self.n_prompts}_lenprompts{self.len_prompts}"
    ...
    def dump_data(self):
        segments = self.basic_task["dataset"]["kwargs"]["segments"] # Benchmark.basic_task()['dataset']['kwargs']['segments']
        t = deepcopy_basic_type(self.basic_task) # copy.deepcopy(self.basic_task)
        ds_train_valid = init_instance_by_config(t["dataset"], accept_types=Dataset) # get the dataset by the configuration
        # dl_train, dl_valid = ds_train_valid.prepare(["train", 'valid'], col_set=["feature", "label"], data_key=DataHandlerLP.DK_L) # get the training and validation data (indeed it conclude all the data for self.basic_task)
        with open(f'/data/liuqiaoan/csi300-DDGDA/csi300_dl_train.pkl', 'rb') as f:
            dl_train = pickle.load(f)
        with open(f'/data/liuqiaoan/csi300-DDGDA/csi300_dl_valid.pkl', 'rb') as f:
            dl_valid = pickle.load(f)
        print(dl_train.get_index())
        ...
        # data = None
        ################################################
        # assert isinstance(data, TSDataSampler)
        ...
        # Only use the `train` and `valid` of the original set for the offline training
        # original
        # --------------------------------------------------------------------------------------------------------------------------
        #    |                                  ||                                  ||                                  |
        # train[0]                      train[1] valid[0]                   valid[1] test[0]                        test[1]

        # rolling_task for offline training
        # --------------------------------------------------------------------------------------------------------------------------
        #    |                  |     |                                     |
        # train[0] train[0]+step-1    train[0]+step-1+trunc_days   valid[1]-step       
        #                        |   |           |
        #                  extra[0] extra[1]     seperate_point
        ...
        # assert isinstance(md_online.meta_task_l[0].get_meta_input()['X_train'], TSDataSampler)
        return (dl_train, dl_valid), md_online

    def offline_training(self, seed):
        ...
        mm = MASTERManager(d_feat=self.d_feat, d_model=self.d_model, t_nhead=self.t_nhead, s_nhead=self.s_nhead,
                            ...
                            n_epochs = self.n_epochs, lr = self.lr, GPU = self.GPU, seed = seed, train_stop_loss_thred = self.train_stop_loss_thred,
    ...
    def run_all(self):
        ...
        for i in range(0, 5):
            ...
            try:
                ...
                mm: MetaModelRolling = rec.load_object("framework")
                ...
                mm = self.offline_training(seed=self.seed + i)
