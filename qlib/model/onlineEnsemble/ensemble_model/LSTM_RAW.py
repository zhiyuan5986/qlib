...
class LSTM(Model):
    ...
    def loss_fn(self, pred, label):
        mask = torch.isfinite(label).all(axis=1) & torch.isfinite(pred).all(axis=1)
        
        return self.ic_loss(pred[mask], label[mask])
    ...
    def fit(
        ...
        y_train['label'] = self.label.loc[y_train.index]
