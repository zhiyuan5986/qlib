...
class WeightGenerator(Model):
    ...
    def loss_fn(self, query_set_data, normParam):
        ...
        try: sig_rev = torch.inverse(sig_cond)
        except: sig_rev = torch.inverse(sig_cond + 1e-5 * torch.eye(len(sig_cond)).to(sig_cond.device))
        logp = - ( torch.log(torch.det(sig_cond)) + torch.sum(((x2 - miu_cond) @ sig_rev) * (x2 - miu_cond), dim=1).mean() )
        
        return -logp, (miu_cond, sig_cond)
    
        
    def metric_fn(self, query_set_data, normParam):
        error = query_set_data[:, self.feature_size:]
        loss, (mius, sig) = self.loss_fn(query_set_data, normParam)
        sigs = [sig + miu.reshape((-1, 1)) @ miu.reshape((1, -1)) for miu in mius]
        eye = torch.eye(len(sigs[0])).to(error.device)
        sig_revs = torch.stack([torch.inverse(s) if torch.linalg.matrix_rank(s) == len(s) else eye for s in sigs])
        w = sig_revs.sum(dim=-1)
        w = F.leaky_relu(w, negative_slope=0.1)
        w /= torch.sum(w, dim=1, keepdim=True)
        
        score = ((error * w).sum(dim=1) ** 2).mean() * 0.5
        avg_score = (error ** 2).mean() * 0.5
    ...
    def train_epoch(self, tasks):
        ...
        for i in tqdm(range(len(indices))[::self.batch_size][:-1]):
            ...
            for k in range(i, i+self.batch_size):
                ...
                loss, _ = self.loss_fn(query_set_data, normParam)
    ...
    def test_epoch(self, tasks):
        ...
        for i in tqdm(range(len(indices))[::self.batch_size]):
            ...
            for k in range(i, min(i+self.batch_size, len(indices))):
                support_set = torch.from_numpy(self.data.loc[tasks[k][0]].values.astype(np.float32)).to(self.device)
                query_set = torch.from_numpy(self.data.loc[tasks[k][1 if k == len(indices)-1 else 2]].values.astype(np.float32)).to(self.device)
                normParam = self.reweight_model(support_set)
                query_set_data = self.reweight_model(query_set, return_normParam=False)
...
class ReweightModel(nn.Module):
    def __init__(self, d_feat=5, hidden_size=64, num_layers=2, dropout=0.0, 
        ...
        init.normal_(self.reweighter.weight, mean=0, std=1e-5)
        self.feature_extractor = nn.Linear(embedding_size, feature_size)
        self.weight_feature_extractor = nn.Linear(embedding_size, embedding_size)
    ...
    def forward(self, data, return_normParam=True):
        ...
        weight = self.reweight(self.weight_feature_extractor(embedding), error)
        del out, embedding
