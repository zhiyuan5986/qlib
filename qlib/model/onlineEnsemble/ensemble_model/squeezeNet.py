...
class Fire1d(nn.Module):
    ...
    def __init__(self, inplanes, squeeze_planes,
                 expand1_planes, expand3_planes):
        super(Fire1d, self).__init__()
        self.inplanes = inplanes
        self.squeeze = nn.Conv1d(inplanes, squeeze_planes, kernel_size=1)
        self.squeeze_activation = nn.ReLU(inplace=True)
        self.expand1 = nn.Conv1d(squeeze_planes, expand1_planes,
                                   kernel_size=1)
        self.expand1_activation = nn.ReLU(inplace=True)
        self.expand3 = nn.Conv1d(squeeze_planes, expand3_planes,
                                   kernel_size=3, padding=1)
        self.expand3_activation = nn.ReLU(inplace=True)
...
class SqueezeNet1d(nn.Module):
    ...
    def __init__(self, in_channels, outsize):
        ...
        # Final convolution is initialized differently from the rest
        final_conv = nn.Conv1d(512, 128, kernel_size=1)
        self.classifier = nn.Sequential(
            nn.Dropout(p=0.5),
            final_conv,
            nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        x = x.reshape(len(x), self.in_channels, -1)
        x = self.features(x)
        x = self.classifier(x)
        return x
